...
static YURI_WORDS: [&'static str; 78] = [
    ...
    "extreme", "fester", "fickle", "flee", "frightening", "graveyard",
...
pub static SAYORI_WORDS_SET: Lazy<HashSet<&'static str>> = Lazy::new(|| {
    SAYORI_WORDS.iter()
                .map(|refe| *refe)
                .collect()
});

pub static YURI_WORDS_SET: Lazy<HashSet<&'static str>> = Lazy::new(|| {
    YURI_WORDS.iter()
                .map(|refe| *refe)
                .collect()
});

pub static NATSUKI_WORDS_SET: Lazy<HashSet<&'static str>> = Lazy::new(|| {
    NATSUKI_WORDS.iter()
                .map(|refe| *refe)
                .collect()
});
